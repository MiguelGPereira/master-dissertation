                           ...
                           maxpairs = maxPairs, Xmx = "2000M", confThreshold = 0.05)
{
  ...
  #opt = paste(c("-RANK", theta, ",",1, " ","-Pair", maxpairs, " ", "-nodefault -nodef -RS4"), collapse="")
      ...
      if (length(rr$Ant)>0 && !is.na(rr$Ant)) {
        ...
        if (length(grep("null", as.character(rr$Cons)))==0){
          ...
          sapply(pairs, function(pair){
            ...
            # } else {
            #   pairsMatrix[first, second] <- 0
            #   pairsMatrix[second, first] <- 0
            # }
          ...
          if(sup==1.3 && conf==100 && lift==327.66){
            #browser()
          }
